...
describe('action', () => {
  ...
  describe('generateQueriesWithUniqueParams', () => {
    ...
    it('should return specified extractor status', async () => {
      ...
      const qb1 = repository
        .createQueryBuilder('block')
        .select('block.height', 'height')
        .where('height > :h1', { h1: 250 });

      const qb2 = dataSource
        .getRepository(BlockEntity)
        .createQueryBuilder('block')
        .select('block.height', 'height')
        .where('height >= :h2', { h2: 200 });

      // console.log(qb2.getQuery());

      // const q3 = {} as SelectQueryBuilder<ObjectLiteral>;
      ...
      const result = await dataSource
        .createQueryBuilder()
        .select('*')
        .from(`(${unionQuery})`, 'height')
        // .setParameters({ ...qb2.getParameters(), ...qb1.getParameters() })
