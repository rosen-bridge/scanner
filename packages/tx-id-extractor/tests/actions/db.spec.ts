...
describe('TxAction', () => {
  ...
  describe('createUsedBlocksQuery', () => {
     ...
     * - Database
     * @scenario
     * - Insert 4 TxIdEntity with two different  `extractorId‍‍` into the table
    ...
    it('should return only the used blocks associated with the input `extractorId`', async () => {
      const sampleTxIdEntities1 = [
        { txId: 'txid1block1', blockId: 'block1', extractor: 'extractor 1' },
        { txId: 'txid2block1', blockId: 'block1', extractor: 'extractor 1' },
      ];

      const sampleTxIdEntities2 = [
        { txId: 'txid1block1', blockId: 'block1', extractor: 'extractor 2' },
        { txId: 'txid2block1', blockId: 'block1', extractor: 'extractor 2' },
      ];

      await repository.insert([...sampleTxIdEntities1, ...sampleTxIdEntities2]);

      const extractorId = sampleTxIdEntities1[0].extractor;
      ...
      const sampleBlocks = sampleTxIdEntities1.map(
